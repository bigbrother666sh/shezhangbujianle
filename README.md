...
![img](https://github.com/bigbrother666sh/shezhangbujianle/blob/main/assets/621647873263_.pic.jpg）
...
记忆机制的实现也并不算复杂，以列表的形式存贮过去若干轮次（本项目最后用的是5轮，主要是浪潮API每次生成输入+输出要求不超过2048个token，记忆内容太多怕超限）的对话，然后把这些对话文本join成一个str代替问题文本进行生成……不过落实到具体编程上，需要分别记忆不同对话的内容，
记忆机制的实现也并不算复杂，以列表的形式存贮过去若干轮次（本项目最后用的是5轮，主要是浪潮API每次生成输入+输出要求不超过2048个token，记忆内容太多怕超限）的对话，然后把这些对话文本join成一个str代替问题文本进行生成……不过落实到
