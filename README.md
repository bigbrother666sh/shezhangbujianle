...
实际实现中，因为除AI扮演的角色外游戏还有四个角色，而每个角色针对AI角色的提问又有两种可能：1、私聊中提问，2、公聊（群）里面的@提问，所以语料库被分装成8个TXT文件，程序会根据提问者以及问题来源（私聊还是公聊）去对应选择语料来源。这个机制的思路很简单，但是执行起来马上遇到的一个问题就是，如何从对应语料中抽取与当前提问最为相似的example？因为在实际游戏中，玩家可能的提问方向会很多，为了照顾到各种情况，我们准备的语料库也会涵盖很多问题（截止本项目发布时，8个语料库文件最少的问题量也达到了
个），显然不可能把这些问题都作为example在每次生成时输给模型。在这里我们用到了百度飞桨@PaddlePaddle 发布的预训练模型——**simnet_bow**
...
对于这个问题，团队最终采取了一个非常简单粗暴的方案：编导组除主编外每人负责一个角色（刚好四人），自己没事儿就假装在玩这个游戏，想象看会跟AI提什么问题，然后再切换到AI的角度，思考合适的回答……初始语料文件好了之后，大家交换角色进行体验，每次体验后更新各自负责的语料库文件；之后公测也是一样，每轮之后编导组都会根据当场AI回答得比较差的问题进行语料库的完善和补充……为此我们在程序中增加了一个功能：程序会把本场用户的每次提问，以及对应抽取出的三个example问题的simnet_bow相似度得分，并源1.0最终生成的回答文本，按语料库对应另存为8个文本文件，以便于编导们针对性更新语料库（本项目目前开源提供的语料库是截止
，公测 轮后的版本）。

# 记忆机制

本来这个项目一开始是不打算引入记忆机制的，因为我们看源1.0在合适example的few-shot下生成效果已经很不错了，就琢磨着偷点懒。直到最后一轮内测的时候，青玲同学提出要是有人只是@了AI但是没有说话怎么办？我仔细想了下，发现这个出现的频率还未必低，因为很多人（包括我）有时会习惯先把问题打出来，然后再单独发消息@，尤其是@很多人的时候……而一旦发生这个情况，等于本次传入的问题文本就是None，这别说正常生成了，甚至会直接导致程序崩溃……这里稍微再跑个题，其实别看只是实现文本对话，你放到微信这个场景里面，其实用户的聊天会有多种可能，比如有人习惯用空格代替逗号，甚至是频繁的换行……这些对于人类来说都没所谓，但是对于AI来说，轻则导致生成异常（因为AI把输入的都视为有意义的），重则直接导致程序bug……所以对这些输入文本进行预处理花费了我很多精力，当然这个过程也学会了很多，比如python的正则表达式，虽然烧脑但还挺有趣的。

接着上面继续说，这种只@，但没有具体信息的情况，我们虽然可以最简单粗暴的用一个固定的文本（比如“嗯？”）进行替代，但这样这句话的回复肯定是不理想的，人类在这种情况下，是会看上文，然后再想如何回答的。所以最后这一点迫使我们下定决心引入记忆机制。

记忆机制的实现也并不算复杂，以列表的形式存贮过去若干轮次的对话，然后把这些对话文本join成一个str代替问题文本进行生成……但我其实一开始很担心这个机制会和我们的example机制造成冲突，因为example都是“一问一答”，并非连续的对话，我担心这样的话example的提示作用会大幅降低，然而实践下来却并！没！有！反而，效果经验，模型具有了一定的“逻辑能力”，之前最薄弱的指代混乱（他、她代表的具体人物）也得到了很大的改善！**
这让我们感觉到对于多轮对话来说（无疑“目的性对话”就是一种多轮对话），上溯轮次的记忆机制还是很有必要的！**

不过，这种记忆机制也有一些坑。我们一开始设定的是AI不仅会记忆上述若干轮次用户的提问，也会记住自己的回答，本意是这样会让生成的信息更加全面，但实践下来有点得不偿失，首先模型依然不能很好的“记住”自己曾经说过的话，其次很容易造成若干轮次后模型开始重复回答，尤其是当前面某一轮次的回答效果不好的情况下（这种情况有点像AI会自己把自己带到坑里去，这一点的原因和机制我目前也在分析中）……所以最后我们调整为只记忆用户说过的话，而不记忆自己说过的话；但这就引入第二个坑，由于记忆是残缺的，而在连续对话中，用户的提问很可能也是基于上一轮AI的回答，所以这很容易造成AI对信息的误解，但是在群聊环境中，由于正常情况下会有很多人参与对话，用户的发问也不会完全基于AI的发言，所以这个情况就会好很多……所以我们最后的方案是私聊只记忆前面两个轮次的用户提问，群聊中记忆前面五个轮次的用户提问。
没有"注意力"机制的记忆机制在私聊中收益并不大，甚至可能是负面的，而群聊环境中如果每一个参与者都很积极的发言，那么这个收益还是可以的。
另外本项目中对内容文本也进行了预处理，比如替换全部的#和&，这是因为浪潮源1.0目前的缺陷，输入文本中包含这两个字符，前者会导致请求不成功，后者会导致结果查询不成功，这两个问题浪潮源团队已经在修正了。
我们目前也是按浪潮源1.0团队的建议，将这两个字符分别替换为%23和%26.

# 导演机制

# 回到作品本身
一个有趣的问题是，AI玩家引入后，本作还是一个"剧本杀"么？

应该说，至少我本人在这个项目最初阶段并不是想做一个"剧本杀"，我更想让AI成为整个作品的主角，毕竟这是一个NLP大模型实践项目。但是我构思了很多种形式，发现要么对现在的AI来说难度太高了，要么就是体验形式其实并不强烈需要AI，而最后当我们确定了
魔改"社长不见了"这个微信剧本杀的思路后，整个体验也就成了"剧本杀"。

但是AI玩家实际表现却充满了"不确定性"，而我们索性也直接在游戏规则中加入了一条："另外如果其他人提到了你所不知道的剧情，请相信那只是没有出现在你的剧本中，而非不存在"，这样就等于让AI可以尽情发挥"加戏"，当然这一切都还是在剧本框架下的，这一点
浪潮源1.0有表现的很有"逻辑自洽性"。相应的，我们也索性直接告诉玩家我们不会提供复盘文本，线上一个小时的"剧本杀"后，他们也可以私下里互相对信息进行复盘，而我们在体验后也不会关掉程序，也就是玩家可以继续找AI进行"复盘"，这就非常有趣，
玩家是真的在"复盘"，但是程序还是在不断"生成"新的剧情，然后我们还贴心的准备了游戏后配套的延展剧情朋友圈素材，这一切让本作成为一部"活着的故事"，是一部由玩家和AI在不知不觉中共同创造的故事，我将之称为"交互式叙事"。

下面是玩家（角色为谭明）在"剧本杀"流程结束后与AI的复盘对话，AI在原有故事框架下又编出了很多合理而又脑洞大开的情节。

下图是我们配套的延展剧情朋友圈，会在"剧本杀"结束后3天内陆续由AI账号发出，这很类似"规则怪谈"。

可以说本作最好玩的部分恰恰是在"剧本杀"流程结束后才刚开始。
