...
记忆机制的实现也并不算复杂，以列表的形式存贮过去若干轮次的对话，然后把这些对话文本join成一个str代替问题文本进行生成……但我其实一开始很担心这个机制会和我们的example机制造成冲突，因为example都是“一问一答”，并非连续的对话，我担心这样的话example的提示作用会大幅降低，然而实践下来却并！没！有！反而，效果经验，模型具有了一定的“逻辑能力”，之前最薄弱的指代混乱（他、她代表的具体人物）也得到了很大的改善！**这让我们感觉到对于多轮对话来说（无疑“目的性对话”就是一种多轮对话），上溯轮次的记忆机制还是很有必要的！**
